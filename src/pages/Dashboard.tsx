...
import { Meeting, MeetingStatus, UserRole } from '../types';
...
const Dashboard: React.FC = () => {
  ...
  const [viewScope, setViewScope] = useState<'all' | 'participant' | 'notulis' | 'host'>('all');
  ...
  const filteredMeetings = useMemo(() => {
    ...
    if (viewScope === 'participant' && user) {
        relevantMeetings = meetings.filter(meeting => 
            meeting.participants.some(p => p.id === user.id)
        );
    } else if (viewScope === 'notulis' && user) {
        relevantMeetings = meetings.filter(meeting => meeting.notulis.id === user.id);
    } else if (viewScope === 'host' && user) {
        relevantMeetings = meetings.filter(meeting => meeting.createdBy.id === user.id);
    }
    ...
    <div className="container mx-auto">
      ...
      <div className="mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center bg-gray-200 dark:bg-gray-700 rounded-lg p-1">
              ...
              <button onClick={() => setViewScope('participant')} className={`px-4 py-1 rounded-md text-sm font-medium ${viewScope === 'participant' ? 'bg-white dark:bg-gray-800 shadow dark:text-gray-100' : 'text-gray-600 dark:text-gray-300'}`}>Me as Participant</button>
              <button onClick={() => setViewScope('host')} className={`px-4 py-1 rounded-md text-sm font-medium ${viewScope === 'host' ? 'bg-white dark:bg-gray-800 shadow dark:text-gray-100' : 'text-gray-600 dark:text-gray-300'}`}>Me as Host</button>
              <button onClick={() => setViewScope('notulis')} className={`px-4 py-1 rounded-md text-sm font-medium ${viewScope === 'notulis' ? 'bg-white dark:bg-gray-800 shadow dark:text-gray-100' : 'text-gray-600 dark:text-gray-300'}`}>Me as Notulis</button>
          </div>
          <input
            type="text"
